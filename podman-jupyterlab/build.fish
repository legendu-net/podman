...
function _show_help
    ...
    echo ""
    echo "Build the legendu/jupyterlab image from legendu/python-nodejs:next."
    echo ""
    echo "Options:"
    echo "  --no-pull    Skip pulling quay.io/legendu/python-nodejs:next"
    echo "  --no-push    Skip pushing quay.io/legendu/jupyterlab:next"
...
if set -q _flag_no_cache
    podman build --no-cache -t quay.io/legendu/jupyterlab:next .
else
