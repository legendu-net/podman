...
function _show_help
    ...
    echo ""
    echo "Build the legendu/python-nodejs image from legendu/python:next."
    echo ""
    echo "Options:"
    echo "  --no-pull    Skip pulling quay.io/legendu/python:next"
    echo "  --no-push    Skip pushing quay.io/legendu/python-nodejs:next"
...
if set -q _flag_no_cache
    podman build --no-cache -t quay.io/legendu/python-nodejs:next .
else
