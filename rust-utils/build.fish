...
function _show_help
    ...
    echo ""
    echo "Build the legendu/rust-utils image from legendu/rust:next."
    echo ""
    echo "Options:"
    echo "  --no-pull    Skip pulling quay.io/legendu/rust:next"
    echo "  --no-push    Skip pushing quay.io/legendu/rust-utils:next"
...
if set -q _flag_no_cache
    podman build --no-cache -t quay.io/legendu/rust-utils:next .
else
